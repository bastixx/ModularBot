...
def convert(s, message):
    ...
    match = re.search('([0-9,\.]+)ft([0-9,\.]+)in', messagesplit[1], flags=re.I)
    if match:
        feet, inches = float(match.group(1)), float(match.group(2))
        cm = (feet * 30.48) + (inches * 2.54)
        send_message(s, f"{messagesplit[1]} is {cm} cm!")
    ...
    match = re.search('([0-9,\.]+)f', messagesplit[1], flags=re.I)
    if match:
        fahrenheit = float(match.group(1))
        celsius = (fahrenheit - 32) / 1.8
        send_message(s, f"{fahrenheit} °F is {round(celsius, 2)} °C!")

    match = re.search('([0-9,\.]+)c', messagesplit[1], flags=re.I)
    if match:
